...
extern volatile char IOT_Process_Tx[4][SMALL_RING_SIZE];
extern volatile char USB_Ring_Rx[SMALL_RING_SIZE];
extern volatile char USB_Process_Tx[4][SMALL_RING_SIZE];
...
void process_UCA0(void) {
    if(displayflag0 == HIGH) {
          ...
          for (j = 0; j < 10; j++) {
              if(IOT_Process_Tx[j] == '\r') {
                  break;
              }
              else {
                  display_line[3][j] = IOT_Process_Tx[j];
                  display_changed = TRUE;
                  update_display = TRUE;
              }
          }
