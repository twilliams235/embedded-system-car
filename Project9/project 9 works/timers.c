...
volatile char IOT_Ring_Rx[SMALL_RING_SIZE] = " NCSU 1 ";
    ...
    if (P2IFG & SW2) {
        ...
        for (i = 0; i < 10; i++) {
            if(USB_Process_Tx[i] == '\r') {
                ...
                display_line[1][i] = USB_Process_Tx[i];
                display_changed = TRUE;
                update_display = TRUE;
             }
         }
...
// ls /dev/ | grep cu
// picocom /dev/cu.usbmodem212303 --baud 115200 --omap crcrlf --echo
...
__interrupt void eUSCI_A0_ISR(void){ //This interrupt is the interrupt relating to serial communication port UCA0
    ...
    case 0: break;
    case 2:{
        strcpy(display_line[0], " Received0");
        display_changed = TRUE;
        update_display = TRUE;
    ...
    case 4:{
        if(no_transmit == HIGH) {
            UCA1TXBUF = USB_Process_Tx[direct_iot];
            USB_Process_Tx[direct_iot]  = 0;
            direct_iot++;
            if(direct_iot >= sizeof(USB_Process_Tx)){
                direct_iot = BEGINNING;
            }
            if(USB_Process_Tx[direct_iot] == 0x00){
                UCA0IE &= ~UCTXIE;
            }
        }
    }break;
...
__interrupt void eUSCI_A1_ISR(void){ //This interrupt is the interrupt relating to serial communication port UCA1
    ...
    case 0: break;
    case 2:{
        strcpy(display_line[0], " Received1");
        display_changed = TRUE;
        update_display = TRUE;
        no_transmit = HIGH;
        usb_value = UCA1RXBUF;  //UCA1RXBUF has a flag that is set true when it holds a value (Ring buffer must be full?)
    ...
    case 4:{
        if(no_transmit == HIGH) {
            UCA0TXBUF = IOT_Process_Tx[iot_tx];
            IOT_Process_Tx[iot_tx] = 0;
            iot_tx++;
            if(iot_tx >= sizeof(IOT_Ring_Rx)){
                iot_tx = BEGINNING;
            }
            if(IOT_Process_Tx[iot_tx] == 0x00){
                UCA1IE &= ~UCTXIE;
            }
        }
    }break;
