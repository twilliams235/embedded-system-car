...
extern unsigned int displayflag;
volatile char USB_Process_Rx[SMALL_RING_SIZE];
unsigned int baud;
    ...
    if (P2IFG & SW2) {
        ...
        for (i = 0; i < 10; i++) {
            if(USB_Ring_Rx[i] == '\r') {
                ...
                display_line[1][i] = USB_Ring_Rx[i];
            }
         }
        strcpy(display_line[0], " Transmit ");
        display_changed = TRUE;
        update_display = TRUE;
        //IOT_Ring_Rx[SMALL_RING_SIZE] = "460,800";
//        strcpy(display_line[0], " 460,800 ");
//        strncpy(IOT_Ring_Rx, " UNCA #2 ", 10);
    }
//
}
