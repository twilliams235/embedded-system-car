...
void Init_Ports() {                 // Initialize all the ports
    Init_Port1();
    Init_Port2();
    Init_Port3();
    Init_Port4();
...
void Init_Port1(void) {
    ...
    //pin 1
    P1SELC |= V_A1_SEEED;          // ADC input for V_A1_SEEED
...
//    P5DIR &= ~V_3_3;
    ...
    P5OUT |= IOT_BOOT;         // Initial Value = High
...
void Init_Port6(void) {
    ...
    //pin 1
    P6SEL0 |= L_REVERSE;       // R_REVERSE GPIO operation
    P6SEL1 &= ~L_REVERSE;       // R_REVERSE GPIO operation
    //P6OUT &= ~L_REVERSE;        // Initial Value = Low / Off
    P6DIR |= L_REVERSE;        // Direction = output

    //pin 2
    P6SEL0 |= L_FORWARD;       // R_FORWARD GPIO operation
    P6SEL1 &= ~L_FORWARD;       // R_FORWARD GPIO operation
    //P6OUT &= ~L_FORWARD;        // Initial Value = Low / Off
    P6DIR |= L_FORWARD;        // Direction = output

    //pin 3
    P6SEL0 |= R_REVERSE;       // L_REVERSE GPIO operation
    P6SEL1 &= ~R_REVERSE;       // L_REVERSE GPIO operation
    //P6OUT &= ~R_REVERSE;        // Initial Value = Low / Off
    P6DIR |= R_REVERSE;        // Direction = output

    //pin 4
    P6SEL0 |= R_FORWARD;       // L_FORWARD GPIO operation
    P6SEL1 &= ~R_FORWARD;       // L_FORWARD GPIO operation
